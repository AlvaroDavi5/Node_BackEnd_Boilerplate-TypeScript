...
Oxlint does not implement formatting rules at all: none of the names below exist in its rule registry. They are now covered by [`.oxfmtrc.json`](../.oxfmtrc.json) or [`.editorconfig`](../.editorconfig), so the _behaviour_ survived even though the rules did not.
