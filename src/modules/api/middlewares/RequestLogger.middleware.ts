...
export default class RequestLoggerMiddleware implements NestMiddleware {
	...
	private maskSensibleData(data: object) {
		...
		const hasSensibleData: boolean = checkFieldsExistence(data, sensibleDataFields as Array<keyof object>);
		if (hasSensibleData) {
			const newData = structuredClone(data);
			return replaceFields(newData, sensibleDataFields as Array<keyof object>, '***');
		}
